...
| Tool                                         | When to use                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `lookup_proxy_cache`                         | Before multi-view; **hit hard-blocks Stage-1 retrieve** (`stage1_bypassed`) → fuse/commit with cached proxies (still abstain before predict). Tools listed in promoted `tools.soft_disabled` are skipped / return `disabled_by_evolution` (offline self-evolution; see `rbp_eval/evolve/` (local `docs/product/SELF_EVOLUTION.md` if present))                                                                                                                                                                                                                       |
...
Guide: local `docs/guides/BATCH_FASTA_SCORE.zh.md` if present (readable via `read_project_doc`); otherwise use `nanobot-bio --help` / `rbp_eval` batch paths.
...
- [ ] Unseen path called seq + struct/domain + Function (`get_func_annotation` + `literature_search`) in parallel intent; set `fusion_weights` from evidence when one axis is decisive; no invented `s_i`/`p_hat`; no motif/annotation from model memory?
